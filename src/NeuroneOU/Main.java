...
class NeuroneSortie {
    ...
    public NeuroneSortie() {
        // Pour la fonction "OU", nous avons deux entrées et un seul neurone de sortie
        poids = new double[] {1, 1}; // Poids pour les deux entrées
        seuil = 0.9; // Seuil arbitraire
    }
...
// Classe représentant le réseau de neurones pour la fonction "OU"
class ReseauNeuronesOU {
    private NeuroneEntree[] entrees; // Neurones d'entrée
    private NeuroneSortie sortie; // Neurone de sortie

    public ReseauNeuronesOU() {
...
public class Main {
    public static void main(String[] args) {
        // Créez un réseau de neurones OU
        ReseauNeuronesOU reseauOU = new ReseauNeuronesOU();

        // Testez le réseau avec différentes combinaisons d'entrées pour la fonction "OU"
        double[][] tests = {{0, 0}, {0, 1}, {1, 0}, {1, 1}};
        for (double[] test : tests) {
            double entree1 = test[0];
            double entree2 = test[1];
            double resultat = reseauOU.propager(entree1, entree2);
